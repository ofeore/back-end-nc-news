...
describe("user_topics table", () => {
  test("user_topics table exists and has data", () => {
    return db.query(`SELECT * FROM user_topics;`).then(({ rows }) => {
      expect(rows.length).toBeGreaterThan(0);
    });
  });

  test("every user_topics row points to a real user and a real topic", () => {
    return db
      .query(
        `
        SELECT ut.user_topic_id
        FROM user_topics ut
        LEFT JOIN users u ON ut.username = u.username
        LEFT JOIN topics t ON ut.topic = t.slug
        WHERE u.username IS NULL OR t.slug IS NULL;
      `,
      )
      .then(({ rows }) => {
  ...
  test("a user cannot follow the same topic twice (unique username + topic)", () => {
    return db
      .query(`SELECT username, topic FROM user_topics LIMIT 1;`)
      .then(({ rows: [{ username, topic }] }) => {
        return db.query(
          `INSERT INTO user_topics (username, topic) VALUES ($1, $2);`,
          [username, topic],
        );
      })
      .then(() => {
        throw new Error("Expected insert to fail due to UNIQUE constraint");
      })
      .catch((err) => {
...
describe("data insertion", () => {
  ...
  test("user_topics data has been inserted correctly", () => {
    return db.query(`SELECT * FROM user_topics;`).then(({ rows }) => {
      ...
      rows.forEach((row) => {
        expect(row).toHaveProperty("user_topic_id");
